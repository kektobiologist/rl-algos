...
import gym
import numpy as np
from policy_gradient.REINFORCEDiscrete import REINFORCEDiscrete
...
def policy_network_slim(states):
    net = slim.stack(states, slim.fully_connected, [24, 24], activation_fn=tf.nn.tanh)
    net = slim.fully_connected(net, num_actions, activation_fn=None)
    return net

pg_reinforce = REINFORCEDiscrete(policy_network_slim,
                              optimizer,
                              sess,
                              num_actions,
                              observation_shape)
